...
const SearchUsersDrawer = ({ isDrawerOpen, setIsDrawerOpen }: Props) => {
  ...
  const searchUsers = debounce(async (e: InputEvent) => {
    ...
    try {
      const { data } = await axios.get(`/api/user?search=${query}`,config as AxiosRequestConfig);
  ...
  const createOrRetrieveChat = async (userId: string) => {
    ...
    const config = getAxiosConfig({ loggedInUser });
    try {
      const { data } = await axios.post(`/api/chat`,{ userId },config as AxiosRequestConfig);
    ...
    <>
      ...
      >
        <div className="d-flex justify-content-center">
          <span
            className="text-center mt-1 user-select-none text-light searchField"
