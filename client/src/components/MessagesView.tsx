...
const MessagesView = ({ loadingMsgs, setLoadingMsgs, setDialogBody, deletePersistedNotifs, isNewUser, typingChatUser}: Props) => {
  const {
    loggedInUser,
    selectedChat,
    selectedCall,
  ...
  const onEmojiClick = ( emoji: EmojiClickData, event: MouseEvent) => {
    if (!msgContent?.current) return;
    msgContent.current.innerHTML += emoji.emoji;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const apiUrl = isNonImageFile ? `/api/message/upload-to-s3` : `/api/message/`;
      console.log(apiUrl);
