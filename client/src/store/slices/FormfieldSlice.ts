...
const initialState: FormfieldState = {
  ...
  formFieldClassName: `app__formfield form-outline`,
  inputFieldClassName: `app__inputfield form-control form-control-lg searchField text-info border-0 ps-2 pt-1`,
  btnSubmitClassName: `btn btn btn-outline-success d-flex justify-content-center align-items-center col-8 fs-4 rounded-pill`,
...
const FormfieldSlice = createSlice({
  ...
  reducers: {
    setLoading: (state, action: PayloadAction<boolean>) => {
      ...
      state.inputFieldClassName = `app__inputfield form-control ${state.disableIfLoading} searchField`;
      state.btnSubmitClassName = `btn btn btn-outline-success${state.disableIfLoading} d-flex justify-content-center align-items-center col-8 fs-4 rounded-pill`;
